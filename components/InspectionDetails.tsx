...
export default function InspectionDetails({ inspectionId }: InspectionDetailsProps) {
  ...
  const getRoomProgress = (room: Room) => {
    ...
    const completedItems = room.items.filter(item => item.checkedAt).length
    return Math.round((completedItems / room.items.length) * 100)
  }

  const getOverallProgress = () => {
    if (rooms.length === 0) return 0
    const totalProgress = rooms.reduce((sum, room) => sum + getRoomProgress(room), 0)
              ...
              return (
                <div key={room.id} className="card hover:shadow-md transition-shadow duration-200">
                  ...
                  <div className="text-sm text-gray-600 mb-4">
                    {room.items.length} inspection items
                    {room.photos.length > 0 && ` • ${room.photos.length} photos`}
                  </div>
